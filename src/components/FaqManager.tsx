...
const FaqManager = ({ resortId }: { resortId: string }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      {adding && (
        <div className="border border-border rounded-lg p-2.5 space-y-2">
          ...
          <div className="flex gap-1">
            ...
            <Button size="sm" variant="ghost" className="text-[10px] h-6" onClick={() => setAdding(false)}>
              Cancelar
            </Button>
          </div>
        </div>
      ...
      {items.map(item => (
        <div key={item.id} className="border border-border rounded-lg p-2.5 flex items-start gap-2">
